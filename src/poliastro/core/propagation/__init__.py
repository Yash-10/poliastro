...
from .farnocchia import farnocchia, farnocchia_classical, farnocchia_coe
...
@jit
def mikkola_classical(k, p, ecc, inc, raan, argp, nu, tof):  # Initially: k, r0, v0, tof
    """Raw algorithm for Mikkola's Kepler solver using classical orbital elements.
...
@jit
def markley_classical(k, p, ecc, inc, raan, argp, nu, tof):
    """Solves the kepler problem by a non iterative method using classical orbital elements. Relative error is
...
@jit
def pimienta_classical(k, p, ecc, inc, raan, argp, nu, tof):
    """Raw algorithm for Adonis' Pimienta and John L. Crassidis 15th order
    polynomial Kepler solver.
...
@jit
def gooding_classical(k, p, ecc, inc, raan, argp, nu, tof, numiter=150, rtol=1e-8):
    """Solves the Elliptic Kepler Equation using classical orbital elements with a cubic convergence and
    accuracy better than 10e-12 rad is normally achieved. It is not valid for
    eccentricities equal or higher than 1.0.
...
@jit
def danby_classical(k, p, ecc, inc, raan, argp, nu, tof, numiter=20, rtol=1e-8):
    """Kepler solver for both elliptic and parabolic orbits based on Danby's
    algorithm.
