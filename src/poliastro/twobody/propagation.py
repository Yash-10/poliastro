...
from poliastro.core.propagation import (
    danby as danby_fast,
    danby_classical as danby_classical_fast,
    farnocchia as farnocchia_fast,
    farnocchia_classical as farnocchia_classical_fast,
    func_twobody,
    gooding as gooding_fast,
    gooding_classical as gooding_classical_fast,
    markley as markley_fast,
    markley_classical as markley_classical_fast,
    mikkola as mikkola_fast,
    mikkola_classical as mikkola_classical_fast,
    pimienta as pimienta_fast,
    pimienta_classical as pimienta_classical_fast,
...
def farnocchia(*elements, **kwargs):
    """Propagates orbit.

    Parameters
    ----------
    Either cartesian or classical:
    1. Cartesian
        k : ~astropy.units.Quantity
            Standard gravitational parameter of the attractor.
        r : ~astropy.units.Quantity
    ...
    if len(elements) == 4:  # Arguments are cartesian
        ...
        k, p, ecc, inc, raan, argp, nu, tofs = elements
        k = k.to(u.km ** 3 / u.s ** 2).value
        tofs = tofs.to(u.s).value
        results = [
            farnocchia_classical_fast(k, p, ecc, inc, raan, argp, nu, tof)
            for tof in tofs
        ]
...
def mikkola(*elements, **kwargs):
    ...
    if len(elements) == 4:
        ...
        tofs = tofs.to(u.s).value
        results = [
            mikkola_classical_fast(k, p, ecc, inc, raan, argp, nu, tof) for tof in tofs
        ]
...
def markley(*elements, **kwargs):
    ...
    if len(elements) == 4:
        ...
        tofs = tofs.to(u.s).value
        results = [
            markley_classical_fast(k, p, ecc, inc, raan, argp, nu, tof) for tof in tofs
        ]
...
def pimienta(*elements, **kwargs):
    ...
    if len(elements) == 4:
        ...
        tofs = tofs.to(u.s).value
        results = [
            pimienta_classical_fast(k, p, ecc, inc, raan, argp, nu, tof) for tof in tofs
        ]
...
def gooding(*elements, numiter=150, rtol=1e-8):
    ...
    if len(elements) == 4:
        ...
        tofs = tofs.to(u.s).value
        results = [
            gooding_classical_fast(k, p, ecc, inc, raan, argp, nu, tof, numiter, rtol)
            for tof in tofs
        ]
...
def danby(*elements, rtol=1e-8, numiter=20):
    ...
    if len(elements) == 4:
        ...
        tofs = tofs.to(u.s).value
        results = [
            danby_classical_fast(k, p, ecc, inc, raan, argp, nu, tof, numiter, rtol)
            for tof in tofs
        ]
...
def propagate(
    orbit, time_of_flight, *, method=farnocchia, classical=False, rtol=1e-10, **kwargs
):
